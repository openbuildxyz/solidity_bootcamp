...
contract VRFTask is VRFConsumerBaseV2 {
    ...
    uint16 constant REQUEST_CONFIRMATIONS = 5;
    ...
    override
    {
        ...
        for (uint i = 0; i < NUM_WORDS - 1; i++) {
            bytes32 random = keccak256(abi.encodePacked(_randomWords[i]));
            uint selected_item = uint(random) % NUM_WORDS;

            // Swap items selected_item and  i.
            uint aux = s_randomWords[i];
            s_randomWords[i] = s_randomWords[selected_item];
